...
class AmenitiesRequestForm(models.Model):
    ...
    amenity_name = fields.Char(compute="_compute_amenity_name_label", store=False)

    def _compute_amenity_name_label(self):
        for rec in self:
            rec.amenity_name = dict(rec._fields["amenity"].selection).get(rec.amenity)

    requestor_id = fields.Many2one(
        "res.partner", string="Name of Requestor", required=True
    )
    activity_type = fields.Text(string="Type of Activity", required=True)
    sponsor_id = fields.Many2one(
        "res.partner",
        string="If sponsored, unit owner's name",
    )
    relation = fields.Char(string="Relation to member/resident", required=True)
    ...
    def _get_template(self):
        ...
        if not template:
            ...
            pdf_content, _ = self.env["ir.actions.report"]._render_qweb_pdf(
                "pms.action_report_amenities_sign_default", self.ids
            )
