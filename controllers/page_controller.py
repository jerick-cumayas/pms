...
class WebsiteHomeRedirect(http.Controller):
    ...
    def homepage(self, **kwargs):
        ...
        events = (
            ...
            .sudo()
            .search([], order="date_begin asc", limit=3)
        )
        ...
        blogs = (
            ...
            .sudo()
            .search(
                [("website_published", "=", True)], order="post_date desc", limit=10
            )
        )
...
class BlogUserOnly(WebsiteBlog):
    @http.route(
        ["/blog", '/blog/<model("blog.post"):post>'],
        type="http",
        auth="user",
        website=True,
    )
    def blog(self, post=None, tag=None, author=None, **kwargs):
        ...
        return super(BlogUserOnly, self).blog(
            post=post, tag=tag, author=author, **kwargs
        )
